...
zone_map = {
    ...
    "a4": {
        ...
        "grassland",
        DESCRIPTION:
        "You're in beautiful sea of golden grass ",
        EXAMINE:
        "You take a moment to appreciate \nthe natural beauty of the area",
        UP:
        "a4",
        DOWN:
        "b4",
    ...
    "a5": {
        ...
        "grassland",
        DESCRIPTION:
        " Gazelle graze on the abundant grass",
        EXAMINE:
        "You hear a snarl from behind you \nyou also remembered lions live in grasslands",
        UP:
        "a5",
        DOWN:
        "b5",
    ...
    "a7": {
        ...
        "grassland",
        DESCRIPTION:
        "Fields of glorious golden grass",
        EXAMINE:
        "you find a strange black rock that's warm to the touch \nmaybe its worth something at a trading post",
        UP:
        "a7",
        DOWN:
        "b7",
    ...
    "c1": {
        ...
        "forest",
        DESCRIPTION:
        "something seems off about this part of the forest ",
        EXAMINE:
        "upon closer inspection you notice the trees all have a poisonous fungus on them",
        UP:
        "b1",
        DOWN:
        "d1",
    ...
    "d3": {
        ...
        "dark forest",
        DESCRIPTION:
        "the sun doesn't make it through the foliage",
        EXAMINE:
        "the only thing that grows here is mangled trees and thorn bushes",
        UP:
        "c3",
        DOWN:
        "e3",
    ...
    "h8": {
        ...
        "hidden shop",
        DESCRIPTION:
        "this shop caries many different wares \nfrom all around the world",
        EXAMINE:
        "hidden shop",
        UP:
        "g8",
...
def battle():
    enemies = [
        "wolf", "gnome", "ogre", "snake", "dragon", "metal-man",
        "rouge automaton", "tin giraffe", "dark orb", "king crab", "slime",
        "gorgon the great", "average joe"
    ]
...
def attack_processor(word, player_turn, enemy_name):
...
shop_inventory = {shop: [], potion: [], weapon: [], hidden: []}
...
def print_location(action):
    ...
    ypos = alph.index(player1.location[0])
    ...
    xpos = int(player1.location[1]) - 1
    player_map_unsolved[ypos][xpos] = player_map_solved[ypos][xpos]
    if action == "map":

        print_map = ""
        for column in player_map_unsolved:
            for row in column:
                print_map += f"|{row}"
            print_map += "|\n"
        print(print_map)
...
def prompt():
    ...
    acceptable_actions = [
        "go", "move", "travel", "quit", "help", "examine", "interact", "check",
        "look", "map", "legend", "menu", "stats", "pause"
    ]
...
def player_move(myaction):
    ...
    if destination.lower() in ["up", "north"]:
        dest = zone_map[player1.location][UP]
        movment_handler(dest)
    elif destination.lower() in ["down", "south"]:
        dest = zone_map[player1.location][DOWN]
        movment_handler(dest)
    elif destination.lower() in ["left", "west"]:
        dest = zone_map[player1.location][LEFT]
        movment_handler(dest)
    elif destination.lower() in ["right", "east"]:
        dest = zone_map[player1.location][RIGHT]
        movment_handler(dest)
...
def player_examine(myaction):
    trigers = ["puzzle1", "puzzle2", "puzzle3", "puzzle4"]
    ...
    if solved_places[player1.location]:
        ...
        return
    if zone_map[player1.location][EXAMINE] not in trigers and zone_map[
            player1.location][EXAMINE] not in shops:
...
def setup_game():
    ...
    if player1.type.lower() == "ranger":
        ...
        player1.attacks = ["precision shot", "rapid fire"]
